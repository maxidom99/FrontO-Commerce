...
const ProductForm = () => {
  const [formData, setFormData] = useState({
    ...
    descripcion: '',
    precios: '',
    id_cat: '', // Agregado para almacenar el ID de la categoría seleccionada
    img_product: null,
  });
  ...
  useEffect(() => {
    const Categories = async () => {
      try {
        const categoriesData = await getCategoryRequest();
        ...
        <div className="container flex flex-col items-center justify-center min-h-screen px-6 mx-auto">
          ...
          <form className="w-full max-w-md" onSubmit={handleSubmit}>
            ...
            <label className="flex items-center mt-6">
              <select className="block w-full py-3 text-gray-700 bg-white border rounded-lg px-3 dark:bg-gray-900 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 dark:focus:border-blue-300 focus:ring-blue-300 focus:outline-none focus:ring focus:ring-opacity-40" value={formData.id_cat} onChange={handleInputChange} name="id_cat">
                <option value="">Selecciona una categoría</option>
                {categories.map((category) => (
                  <option key={category.id_cat} value={category.id_cat}>{category.nombre}</option>
                ))}
              </select>
            </label>
            ...
            <div className="relative flex items-center mt-6">
              <input type="text" className="block w-full py-3 text-gray-700 bg-white border rounded-lg px-4 dark:bg-gray-900 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 dark:focus:border-blue-300 focus:ring-blue-300 focus:outline-none focus:ring focus:ring-opacity-40" placeholder="Precio"
